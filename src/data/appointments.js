...
export async function completeAppointment(appointment,method){
  ...
  if(clientId){
    ...
    if(!existing){
      const {error}=await supabase.from("client_payments").insert({client_id:clientId,appointment_id:appointment.id,amount:appointment.price,method,date:fmtDate(new Date())});
