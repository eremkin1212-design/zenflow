...
const HOUR_H = 52;
const GUTTER = 40; // ширина колонки с часами слева (left-10)
...
// Все услуги записи: если их несколько, показываем через запятую.
function serviceText(a) {
const list = (a.appointment_services || []).map((x) => x?.services?.name).filter(Boolean);
...
// Раскладка пересекающихся записей по колонкам: записи, которые идут
// одновременно, встают рядом, а не друг на друга.
function buildLayout(list) {
  ...
  const flush = () => {
    const colEnds = [];
    cluster.forEach((a) => {
      const s = minutes(a.start_time);
      const e = s + Number(a.duration || 0);
      let ci = colEnds.findIndex((endAt) => endAt <= s);
      if (ci === -1) { colEnds.push(e); ci = colEnds.length - 1; } else { colEnds[ci] = e; }
      map[a.id] = { col: ci, total: 1 };
    });
    cluster.forEach((a) => { map[a.id].total = colEnds.length; });
    cluster = [];
    clusterEnd = -1;
  };
  sorted.forEach((a) => {
...
: (dayAppts.length ? Math.max(...apptEnds) + 30 : dayEnd);
  ...
  const contentHeight = Math.max(HOUR_H, ((gridEnd - gridStart) / 60) * HOUR_H);
const gridHeight = contentHeight + 24;
  ...
  const layout = useMemo(() => buildLayout(dayAppts), [dayAppts]);
  const hasOverlap = useMemo(() => Object.values(layout).some((l) => l.total > 1), [layout]);
  ...
  function swipeStart(e) { swiped.current = false; swipe.current = { x: e.clientX, y: e.clientY }; }
  function swipeEnd(e) { if (!swipe.current) return; const dx = e.clientX - swipe.current.x; const dy = e.clientY - swipe.current.y; swipe.current = null; if (Math.abs(dx) < 45 || Math.abs(dx) < Math.abs(dy) * 1.2) return; swiped.current = true; view === "day" ? shiftDay(dx < 0 ? 1 : -1) : shiftWeek(dx < 0 ? 1 : -1); }
  ...
  function openAtTime(e) {
    ...
    if (!dayHours.on && dayAppts.length === 0) return;
  ...
  function dragMove(e) {
    ...
    const top = Math.max(0, Math.min(drag.startTop + delta, contentHeight - h));
  ...
  async function complete(method, amount, discount) {
    if (!selected) return;
    try { const updated = await completeAppointment(selected, method, amount, discount); setAppts((p) => p.map((a) => a.id === selected.id ? updated : a)); setSelectedId(null); setCompleteOpen(false); }
  ...
  async function cancel() {
    ...
    catch { window.alert("Не удалось отменить"); }
  }
