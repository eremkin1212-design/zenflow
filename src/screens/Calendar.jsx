...
export default function Calendar() {
  ...
  useEffect(() => {
    ...
    getClientById(selected.clients.id)
      .then((c) => {
        if (!cancelled) setClientDetail(c);
      })
      .catch(() => {});
  ...
  async function moveSelected(deltaMin) {
    ...
    const oldTime = selected.start_time;

    setAppts((prev) =>
      prev.map((a) =>
        a.id === selectedId ? { ...a, start_time: newTime } : a
      )
    );
    ...
    try {
      ...
      setAppts((prev) =>
        prev.map((a) =>
          a.id === selectedId ? { ...a, start_time: oldTime } : a
        )
      );
    }
  }
  ...
  function topToTime(top, duration) {
    ...
    const clamped = Math.max(
      DAY_START,
      Math.min(snapped, DAY_END - duration)
    );
  ...
  function handlePointerDown(e, a) {
    ...
    const top =
      ((toMinutes(a.start_time) - DAY_START) / 60) * HOUR_H;
    ...
    setDrag((d) =>
      d
        ? {
            ...d,
            currentTop: newTop,
            moved: d.moved || Math.abs(deltaY) > 4,
          }
        : d
    );
  }
  ...
  async function handlePointerUp(e) {
    ...
    setAppts((prev) =>
      prev.map((a) =>
        a.id === id ? { ...a, start_time: newTime } : a
      )
    );
    ...
    try {
      ...
      setAppts((prev) =>
        prev.map((a) =>
          a.id === id ? { ...a, start_time: originalStart } : a
        )
      );
    }
  }
      ...
      <div className="max-w-sm mx-auto relative pb-24">
        ...
        {weekOffset !== 0 && (
          <button
            type="button"
            onClick={goToToday}
            className="block mx-auto mt-1 text-xs font-medium text-[var(--moss)]"
          >
            Сегодня
          </button>
        )}
        ...
        {status === "loading" && (
          <div className="text-sm text-center py-10 text-[var(--ink-soft)]">
            Загружаем записи…
          </div>
        )}
          ...
          <div className="mx-5 mt-5 flex flex-col gap-3">
            {weekDates.map((d, i) => {
              ...
              return (
                <div
                  key={i}
                  className="rounded-2xl p-4 bg-[var(--surface)] border border-[var(--line)]"
                >
                  <div
                  ...
                  <div className="flex flex-col gap-2">
                    {list.map((a) => (
                      ...
                      >
                        ...
                        />
                        <span className="text-xs w-11 shrink-0 font-mono text-[var(--ink-soft)]">
                          {a.start_time}
                        </span>
                        ...
                        <span className="text-xs ml-auto shrink-0 text-[var(--ink-soft)]">
                          {a.services?.name}
                        </span>
                      </button>
                    ))}
            ...
            {appts.filter((a) => a.status !== "cancelled")
              .length === 0 && (
              <div className="text-sm text-center py-6 text-[var(--ink-soft)]">
        ...
        {status === "ready" && view === "day" && (
          ...
          >
            {Array.from({
              ...
              <div
                key={i}
                className="absolute left-10 right-0 flex items-start border-t border-[var(--line)]"
                style={{ top: i * HOUR_H }}
              >
            ...
            {isToday(weekDates[dayIndex]) &&
              ...
              nowMinutes <= DAY_END && (
                <div
                  ...
                  style={{
                    top:
                      ((nowMinutes - DAY_START) / 60) *
                      HOUR_H,
                  }}
                >
                  <span
                    className="zf-now-dot w-2 h-2 rounded-full -ml-1"
            ...
            {dayAppts.map((a) => {
              ...
              const top = isDragging
                ? drag.currentTop
                : ((toMinutes(a.start_time) - DAY_START) /
                    60) *
                  HOUR_H;

              const height = Math.max(
                (a.duration / 60) * HOUR_H,
                40
              );

              const isSel = selectedId === a.id;
              const color = a.services?.color || "#999";
              const liveTime = isDragging
                ? topToTime(drag.currentTop, a.duration)
                : a.start_time;
              ...
              return (
                ...
                >
                  <div className="text-xs font-medium pr-6">
                    {liveTime} ·{" "}
                    {a.clients?.name || "Клиент удалён"}
                  </div>

                  <div className="text-[11px] text-[var(--ink-soft)] pr-6">
                    {a.services?.name}
                  </div>

                  <div
                    onPointerDown={(e) =>
                      handlePointerDown(e, a)
                    }
        ...
        {selected && view === "day" && (
          <>
            ...
            <div
              className="fixed left-0 right-0 bottom-0 max-w-sm mx-auto z-30 rounded-t-3xl px-5 pt-5 pb-7 max-h-[85vh] overflow-y-auto"
              style={{
            ...
            >
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {clientDetail && (
                      <span
                        className="inline-block mt-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium"
                        style={{
                          background:
                            ratingTag(clientDetail)
                              .tone === "moss"
                              ? "var(--moss)"
                              : ratingTag(clientDetail)
                                  .tone === "clay"
              ...
              <div className="flex items-center gap-2 mt-4">
                ...
                <button
                  className="rounded-full p-3 bg-[var(--surface-alt)]"
                  aria-label="Написать"
                >
                ...
                <Link
                  to={`/clients/${selected.clients?.id}`}
                  className="flex-1 rounded-full py-3 text-sm font-medium flex items-center justify-center gap-1 bg-[var(--surface-alt)]"
                >
                  Полная карточка
              ...
              {clientDetail && (
                <div className="grid grid-cols-4 gap-2 mt-4">
                  {[
                    {
                      Icon: CalendarCheck,
                      value: clientDetail.visits,
                      label: "визитов",
                    },
                    {
                      Icon: Ban,
                      value: clientDetail.cancellations,
                      label: "отмен",
                    },
                    {
                      Icon: Wallet,
                      value: `${clientDetail.avg_check.toLocaleString(
                        "ru-RU"
                      )}₽`,
                      label: "чек",
                    },
                    {
                      Icon: ClockIcon,
                      value:
                        clientDetail.last_visit?.split(",")[0] ||
                        "—",
                      label: "визит",
                    },
                    ...
                    <div
                      key={i}
                      className="rounded-xl p-2 text-center bg-[var(--surface-alt)]"
                    >
                      <Icon
                        size={13}
                        className="text-[var(--moss)] mx-auto"
                      />
                      <div className="text-xs font-medium mt-1 font-mono truncate">
                        {value}
                      </div>
              ...
              >
                ...
                <div className="text-xs text-[var(--ink-soft)] mb-3 pl-4">
                  {selected.start_time}–
                  {toTime(
                    toMinutes(selected.start_time) +
                      selected.duration
                  )}{" "}
                  · {selected.duration} мин
                </div>
                ...
                <div className="flex items-center justify-between rounded-2xl p-3 bg-[var(--surface-alt)]">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      aria-label="На 30 минут раньше"
                      className="rounded-full p-1.5 bg-[var(--surface)]"
                    >
                      <ChevronLeft size={15} />
                    </button>

                    <span className="text-sm font-mono w-11 text-center">
                      {selected.start_time}
                    </span>

                    <button
                      onClick={() => moveSelected(30)}
                      aria-label="На 30 минут позже"
                      className="rounded-full p-1.5 bg-[var(--surface)]"
                    >
                ...
                {confirmingComplete ? (
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <button
                      ...
                      className="rounded-full p-3 flex items-center justify-center bg-[var(--surface-alt)]"
                    ...
                    <Link
                      to={`/appointment/${selected.id}`}
                      className="flex-1 rounded-full py-3 text-sm font-medium flex items-center justify-center gap-1.5"
                      style={{
                        background: "var(--moss)",
                        color: "var(--on-accent)",
                      }}
