...
import { getClientBalance, getClientBalanceTransactions } from "../data/clientBalance";
...
export default function ClientCard() {
  ...
  const [balance, setBalance] = useState(0); const [balanceTransactions, setBalanceTransactions] = useState([]);
  ...
  useEffect(() => {
    ...
    Promise.all([getClientById(id), getHistory(id), getPayments(id), getNotes(id), getRecommendation(id), getClientBalance(id), getClientBalanceTransactions(id)]).then(([c, h, p, n, r, b, bt]) => {
      if (cancelled) return; if (!c) { setStatus("notfound"); return; }
      setClient(c); setHistory(h); setPayments(p); setNotes(n); setRec(r); setBalance(b); setBalanceTransactions(bt); setStatus("ready");
