        ...
        {status === "ready" && (
          <>
            {dueClients.length > 0 && (
              <section className="mx-5 mt-5">
                ...
                <div className="flex flex-col gap-2.5">
                  {dueClients.slice(0, 3).map((client) => <ClientFollowUpCard key={client.id} client={client} followUp={followUps[client.id]} />)}
                </div>
              </section>
            )}
            ...
            <div className="mx-5 mt-2 flex flex-col gap-2.5">
              ...
              {sorted.length === 0 && <div className="text-sm text-center py-8 text-[var(--ink-soft)]">Никого не нашлось</div>}
            </div>
          </>
        )}
